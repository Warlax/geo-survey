...
function doSubjectSearch(callback, queryString) {
$.ajax({
      type: "POST",
      url: "../post/doSubjectSearch.php",  //address of the php code
      data: "queryString="+queryString, // parameter to pass onto the php code.
      success: function(resp){
        // we have the response
        if(resp.indexOf("<!") >0){
	 			resp = resp.substring(0,resp.indexOf("<!"));
...
function doCategorySearch(callback, queryString) {
$.ajax({
      type: "POST",
